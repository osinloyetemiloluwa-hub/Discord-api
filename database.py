...
from sqlalchemy import create_engine, Column, Integer, String, Text, DateTime, Boolean, ForeignKey, Index, JSON
...
class Message(Base):
    ...
    mentions = Column(JSON, nullable=True)  # Native JSON representation
    mentioned_roles = Column(JSON, nullable=True)  # Native JSON representation
...
class Attachment(Base):
    ...
    def to_dict(self):
        return {
            'id': self.id, 'message_id': self.message_id, 'attachment_id': self.attachment_id,
            'filename': self.filename, 'content_type': self.content_type, 'url': self.url,
            'size': self.size, 'proxy_url': self.proxy_url
        }
...
class EditedMessage(Base):
    ...
    edited_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), index=True)

    def to_dict(self):
        return {
            'id': self.id, 'message_id': self.message_id, 'old_content': self.old_content,
            'new_content': self.new_content, 'edited_at': self.edited_at.isoformat() if self.edited_at else None
        }
...
class DeletedMessage(Base):
    ...
    def to_dict(self):
        return {
            'id': self.id, 'message_id': self.message_id, 'channel_id': self.channel_id,
            'channel_name': self.channel_name, 'guild_id': self.guild_id, 'guild_name': self.guild_name,
            'author_id': self.author_id, 'author_name': self.author_name, 'content': self.content,
            'original_timestamp': self.original_timestamp.isoformat() if self.original_timestamp else None,
            'deleted_at': self.deleted_at.isoformat() if self.deleted_at else None,
            'has_attachments': self.has_attachments, 'reply_to_message_id': self.reply_to_message_id
        }
...
@contextmanager
def get_db_session():
    """Context manager for cleaner session lifecycle handling inside the bot."""
    db = SessionLocal()
    try:
        yield db
    finally:
